...
# Note: this is not a full test -- all we're testing here is that the webapp init create command doesn't crash
...
$TEST_TARGET_SO webapp init create --kube-config ${fake_k8s_config_file} --deployment-dir ${test_deployment_dir} --image ${app_image_name} --url https://my-test-app.example.com
