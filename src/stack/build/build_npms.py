...
def command(ctx, include, exclude, force_rebuild, extra_build_args):
    ...
    npm_registry_url_token = config("BPI_NPM_AUTH_TOKEN", default=None)
    ...
    def build_package(package):
        ...
        if not dry_run:
            ...
            # envs = {"BPI_NPM_AUTH_TOKEN": npm_registry_url_token} | ({"BPI_SCRIPT_DEBUG": "true"} if debug else {})
            # but that isn't available in Python 3.8 (default in Ubuntu 20) so for now we use dict.update:
            envs = {"BPI_NPM_AUTH_TOKEN": npm_registry_url_token,
                    "BPI_HOSTED_CONFIG_FILE": "config-hosted.yml"  # Convention used by our web app packages
                    }
            envs.update({"BPI_SCRIPT_DEBUG": "true"} if debug else {})
            envs.update({"BPI_FORCE_REBUILD": "true"} if force_rebuild else {})
            envs.update({"BPI_CONTAINER_EXTRA_BUILD_ARGS": extra_build_args} if extra_build_args else {})
            try:
...
def _ensure_prerequisites():
    ...
    if len(images) == 0:
        ...
        print("Please run this command to create it: stack --stack build-support prepare-containers")
        sys.exit(1)
